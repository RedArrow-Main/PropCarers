...
// ── Dummy Properties ─────────────────────────────────────────
const dashboardFallback: Property[] = [
...
export default function DashboardPage() {
  ...
  const { allProperties: apiProps } = usePropertiesData();
  const allProperties = apiProps.length ? apiProps : dashboardFallback;
    ...
    <>
      <style>{`
        ...
        @media (max-width: 1399px) {
          .prop-carousel { grid-template-columns: repeat(3, minmax(0, 1fr)); }
        }

        @media (max-width: 1023px) {
          .prop-carousel { grid-template-columns: repeat(2, minmax(0, 1fr)); }
          .stats-area    { width: 220px; }
        }
        ...
        @media (max-width: 640px) {
          ...
          /* Overview card */
          .overview-card  { padding: 16px; height: auto; }
          .overview-body  { flex-direction: column; }
          .chart-total    { font-size: 18px; }
          .chart-subtitle { margin-bottom: 12px; }
          .chart-with-axis { height: 150px; min-height: 0; }
          .bar-col        { height: 120px; }

          /* Stats row — side by side on mobile */
          .stats-area     { flex-direction: row; width: 100%; height: auto; align-self: stretch; border-radius: 10px; }
          .stat-card      { padding: 12px; gap: 8px; flex-direction: column; align-items: flex-start; }
          .stat-value     { font-size: 15px; }
          .stat-sub       { font-size: 9px; }

          /* Property cards */
          .prop-carousel  { grid-template-columns: 1fr; gap: 12px; }
          .prop-img-wrap  { height: 180px; aspect-ratio: auto; }
